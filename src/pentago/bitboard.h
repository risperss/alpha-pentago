...
class BoardNode {
 public:
  ...
  BoardNode(const std::string& str) : BoardNode(str[1] - '1', str[0] - 'a') {}
...
class BitBoard {
 public:
  ...
  void set(BoardSquare square, int squarePos) {
    std::uint64_t kSquareMask = std::uint64_t(0b0111000101000111)
                                << BoardSquare::shiftValue(squarePos);
...
class Move {
 public:
  ...
  bool nodeOnSquare() const { return squarePos() == node().squarePos(); }

  std::string as_string() {
    return node().as_string() + "-" + std::string(1, '1' + squarePos()) +
